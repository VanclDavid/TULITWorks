...
X_test = data_preprocessing_cnn(test_nez_data,tren_data);
N = size(X_test,1);
...
load Spoken_CNN_4Layer_8F_8F_32H_10.mat;
...
for n=1:N
    ...
    for f=1:F2        
        ...
        for h=1:H2
            temp=temp+filter2(squeeze(CW2(f,h,:,:)),squeeze(convout1(h,:,:)));
        end
        convout2(f,:,:) = temp;
        %Pøiètení biasu:
    ...
    for f=1:F2
        for i=1:convout2x/2
            for ii=1:convout2y/2
                maxpoolout(f,i,ii)=max(max(squeeze(convout2(f,(i+i-1):(i+i),(ii+ii-1):(ii+ii)))));
            end
        end        
    end
