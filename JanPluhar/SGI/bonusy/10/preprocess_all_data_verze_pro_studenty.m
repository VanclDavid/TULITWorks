...
for n = 1:pocet_nahravek 
    ...
    if prvni_segment < 1
        silence=sig(posledni_segment*pocet_vzorku_v_segmentu-(1-prvni_segment)*pocet_vzorku_v_segmentu:posledni_segment*pocet_vzorku_v_segmentu);
        extented=zeros(1,length(silence)+length(sig));
        extented(1:length(silence))=silence;
        extented(length(silence)+1:end)=sig;
        sig=extented;
        posledni_segment=posledni_segment+(1-prvni_segment);
        prvni_segment=1;
    end
    ...
    vyrez = filter ([1 0],[1 -1],vyrez_orig);
    ...
    for i = 1:pocet_segmentu_ve_vyrezu
       ...
       %end
       prevzorkovane_amp_spektrum=((amp_spektrum(1:2:end)+amp_spektrum(2:2:end))/2)';
    ...
    % .....
    fileID = fopen('Priznaky.txt','w'); %otevreni seznamu, ktery si musite vytvorit
    for i = 1:size(priznaky_pro_rozpoznavani,1)
        for j = 1:size(priznaky_pro_rozpoznavani,2)
            fprintf(fileID,'%s', num2str(priznaky_pro_rozpoznavani(i,j),'%08.4f')+" ");
        end
        fprintf(fileID,'\r\n');
    end
    fprintf(fileID,'\r\n');
    ...
    %do pole spgram se ukladájí data od zadu, aby byl spektogram orientován obrácenì
    ...
    %tím že je první èíslo vìtší než druhé se invertuje barva
    %reprezentující malé a velké hodnoty
    %pokud nastavíme hodnoty na 0.1 a 0 tak se zvýrazní více hodnot na maximální úroveò barvy
    %pokud dáme hodnoty 1 0 tak tím posuneme hranici pro hodnoty a tím snížíme "kontrast" grafu
    ...
    souboryChars=convertStringsToChars(soubory(n, :))
    souboryChars(k-11)
    souboryChars(k-8:k-5)
    souboryChars(k-2:k-1)
    trida = double(str2num(souboryChars(k-11))); % identifikator cislice nam bude slouzit pro rozpoznavani
    person = double(str2num(souboryChars(k-8:k-5)));  % identifikator osoby
    sada = double(str2num(souboryChars(k-2:k-1)));    % identifikator sady
