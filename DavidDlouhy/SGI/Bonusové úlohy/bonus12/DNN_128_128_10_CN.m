...
x = double(squeeze(test_data (i,:,:))); % naèti test. obrázek
x=Zarovnani(x);
test_data(i,:,:)=[x];
end
for i = 1:9000 % cyklus pro test. obrázky, N max.1000
x = double(squeeze(tren_data (i,:,:))); % naèti test. obrázek
x=Zarovnani(x);
tren_data(i,:,:)=[x];
end
% ========================== VÁŠ KÓD ZDE ======================
...
scores1=max(0,scores1);
%Rozšíøení vypoètené matice o vektor jednièek:
s1scores=ones(1, size(scores1,1));
scores1=[scores1 s1scores'];
...
scores2=max(0,scores2);
%Rozšíøení vypoètené matice o vektor jednièek:
s2scores=ones(1, size(scores2,1));
scores2=[scores2 s2scores'];
...
[maxscore predicts]=max(scores3, [],2);
