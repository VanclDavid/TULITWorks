...
echo = [0.5 555]; %dozvuk hlasitost|posunutí
...
%% Note creation
n = ["p","g0", "gis0", "a0", "ais0", "h0", "c1", "cis1", "d1", "dis1", "e1", "f1", "fis1", "g1", "gis1", "a1", "ais1", "h1", "c2", "cis2", "d2", "dis2", "e2"]; % table of notes
nPos = @(note)(find(~cellfun(@isempty,regexpi(n,note))));
q = 2^(1/12); a1 = 440;
f = @(tLevel)((q.^tLevel) * a1);
nFreq = @(note)(f( nPos(note) - nPos("a1") ));
